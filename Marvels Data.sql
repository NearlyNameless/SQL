...
/*Inserts the inputed values into table marvels*/    
...
/*Selects all infomation from table marvels*/
SELECT * FROM marvels;

/*Counts the data in the table that are alignment = good and gender = female */
SELECT COUNT(*) as Heroine FROM marvels
WHERE alignment = 'Good'
AND gender = 'Female';

/*Gets the average of the height from the table*/
SELECT ROUND(AVG(height_m), 1) as Average_height_Meters FROM marvels;

/*Groups by name and shows only people with a power level bigger than 30*/
SELECT name, SUM(intelligence+strength+speed+durability+energy_Projection+fighting_skills) as Power_Level FROM marvels
GROUP BY name
HAVING SUM(intelligence+strength+speed+durability+energy_Projection+fighting_skills)> 30;

/*Gives a count how many people are good or bad*/
SELECT COUNT (*), 
    CASE
...
/*Selects the strength and adds various names to them depending on the value of strength*/
SELECT name, 'Overall_Strength' = CASE
        WHEN strength  > 6 THEN 'Strong'
        WHEN strength  > 5 THEN 'Above Average'
        WHEN strength  > 4 THEN 'Average'
        WHEN strength  > 3 THEN 'Below Average'
        ELSE 'Weak'
        END
   FROM marvels
   ORDER BY name;
